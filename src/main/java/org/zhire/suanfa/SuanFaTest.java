...
public class SuanFaTest {
    ...
    public void testcurmap() {
        ...
        int[] sum = twoSum(nums, 929);
        System.out.println("符合要求的数组下标为：" + Arrays.toString(sum));
    ...
    public int[] twoSum(int[] nums, int target) {
        System.out.println("输入的值为：" + Arrays.toString(nums));
        System.out.println("目标值为：" + target);
        int[] r = new int[2];
        for (int i = 0; i < nums.length; i++) {
            // System.out.println("外层循环：" + nums[i]);
            for (int length = nums.length; length > 0; length--) {
                //  System.out.println(" 内层循环：" + nums[length - 1]);
                r[0] = i;
                r[1] = length - 1;
                if (nums[i] + nums[length - 1] == target && r[0] != r[1]) {
                    return r;
                }
            }
        }
